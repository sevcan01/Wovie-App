        ...
        <View className='mb-8 space-y-4'>
            <View className='mx-4 flex-row justify-between items-center'>
                ...
                {!hideSeeAll && <TouchableOpacity>
                    <Text style={styles.text} className='text-lg'>{i18n.t('common.seeAll')}</Text>
                </TouchableOpacity>}
                    ...
                    return (
                        <TouchableWithoutFeedback key={index} onPress={() => navigation.navigate('Movie', item)}>
                            ...
                            <View className='space-y-1 mr-4'>
                                <Image source={{ uri: item.poster_path ? `https://image.tmdb.org/t/p/w185/${item.poster_path}` : fallbackPoster }} style={{ width: width * 0.33, height: height * 0.22 }} className='rounded-3xl' />
                                <Text className='text-neutral-900 ml-1'>{item?.title.length > 18 ? item?.title.slice(0, 18) + '...' : item?.title}</Text>
                            </View>
